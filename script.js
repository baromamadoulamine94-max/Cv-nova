...
function initTemplateSelector() {
    ...
    if (savedTemplate) {
        const btn = document.querySelector(`[data-template="${savedTemplate}"]`);
        if (btn) btn.click();
    }
}
...
function updateProgress() {
    ...
    let progress = (filledFields / fields.length) * 60; // 60% pour les infos de base
    
    // Photo = +10%
    if (state.photoData) progress += 10;
    
    // Compétences = +10%
    const hasSkills = Array.from(document.querySelectorAll('.skill-name')).some(i => i.value.trim() !== '');
    if (hasSkills) progress += 10;
    
    // Expériences = +10%
    const hasExp = Array.from(document.querySelectorAll('.exp-title')).some(i => i.value.trim() !== '');
    if (hasExp) progress += 10;
    
    // Éducation = +10%
    const hasEdu = Array.from(document.querySelectorAll('.edu-degree')).some(i => i.value.trim() !== '');
    if (hasEdu) progress += 10;
...
function initDownloadPDF() {
    document.getElementById('download-pdf').addEventListener('click', () => {
        ...
        const opt = {
            margin: 0,
            filename: `CV_${document.getElementById('fullName').value || 'Nova'}.pdf`,
...
function saveToLocalStorage() {
    ...
    localStorage.setItem('cv-nova-data', JSON.stringify(data));
}

function loadFromLocalStorage() {
    const saved = localStorage.getItem('cv-nova-data');
    if (!saved) return;
    ...
    try {
        ...
        if (data.photoData) {
            ...
            document.getElementById('preview-img').src = data.photoData;
            document.getElementById('photo-preview').classList.remove('hidden');
            document.getElementById('upload-zone').classList.add('hidden');
            document.getElementById('cv-photo-img').src = data.photoData;
            document.getElementById('cv-photo-img').style.display = 'block';
            document.querySelector('.cv-photo-placeholder').style.display = 'none';
        }
        
        // Restaurer le template
        if (data.template) {
            const btn = document.querySelector(`[data-template="${data.template}"]`);
            if (btn) btn.click();
        }
        ...
        if (data.skills && data.skills.length > 0) {
            ...
            data.skills.forEach(skill => {
                ...
                lastRow.querySelector('.skill-name').value = skill.name;
                lastRow.querySelector('.skill-level').value = skill.level;
                lastRow.querySelector('.skill-value').textContent = skill.level + '%';
            });
        }
        
        // Restaurer les expériences
        if (data.experiences && data.experiences.length > 0) {
            ...
            data.experiences.forEach(exp => {
                ...
                lastCard.querySelector('.exp-title').value = exp.title;
                lastCard.querySelector('.exp-company').value = exp.company;
                lastCard.querySelector('.exp-date').value = exp.date;
                lastCard.querySelector('.exp-desc').value = exp.desc;
            });
        }
        ...
        if (data.educations && data.educations.length > 0) {
            ...
            data.educations.forEach(edu => {
                ...
                const cards = document.querySelectorAll('.education-card');
                const lastCard = cards[cards.length - 1];
                lastCard.querySelector('.edu-degree').value = edu.degree;
                lastCard.querySelector('.edu-school').value = edu.school;
        ...
        if (data.languages && data.languages.length > 0) {
            ...
            data.languages.forEach(lang => {
                ...
                lastRow.querySelector('.lang-level').value = lang.level;
            });
        }
